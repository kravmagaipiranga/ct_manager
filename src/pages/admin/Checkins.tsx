...
export default function Checkins() {
  ...
  const formattedCheckins = useMemo(() => {
  ...
  }, [checkins, students, classes]);

  const filteredCheckins = useMemo(() => {
    return formattedCheckins.filter(c => activeTab === 'PENDING' ? c.status === 'PENDING' : c.status !== 'PENDING');
  }, [formattedCheckins, activeTab]);
  ...
  const totalPages = Math.ceil(filteredCheckins.length / ITEMS_PER_PAGE) || 1;
  ...
  const handleApproveAll = () => {
    filteredCheckins.filter(c => c.status === 'PENDING').forEach(c => approveCheckin(c.id));
  };
      ...
      <div className="flex gap-4 border-b border-krav-border mb-6 shrink-0 justify-between items-center w-full">
        <div className="flex gap-4">
          ...
          >
            ...
            Aguardando ({formattedCheckins.filter(c => c.status === 'PENDING').length})
          </button>
          <button 
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          {activeTab === 'PENDING' && (
            <div className="space-y-4 max-w-3xl mx-auto">
              ...
              {filteredCheckins.length === 0 ? (
                <div className="p-12 text-center text-krav-muted border border-dashed border-krav-border rounded-xl">
                  Nenhum check-in pendente no momento.
                </div>
              ) : (
                paginatedCheckins.map(checkin => (
                  <div key={checkin.id} className="bg-krav-bg border border-krav-border p-4 sm:p-5 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm hover:shadow-md transition-shadow">
                    <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => rejectCheckin(checkin.id)} className="flex-1 sm:flex-none justify-center bg-transparent border border-krav-danger text-krav-danger hover:bg-krav-danger/10 px-4 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors">
          ...
          {activeTab === 'HISTORY' && (
            <div className="space-y-3 max-w-3xl mx-auto">
              {filteredCheckins.length === 0 ? (
                <div className="p-12 text-center text-krav-muted border border-dashed border-krav-border rounded-xl">
                  Nenhum histórico de check-in encontrado.
                </div>
              ) : (
                paginatedCheckins.map(checkin => (
                  <div key={checkin.id} className="bg-krav-bg border border-krav-border p-4 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-2 shadow-sm">
                    <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="p-1.5 text-krav-muted hover:text-krav-danger bg-transparent border border-transparent hover:border-krav-danger hover:bg-krav-danger/10 rounded-md transition-colors"
