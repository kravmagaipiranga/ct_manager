...
export default function Schedule() {
  ...
  const classes = user?.role === 'INSTRUCTOR' 
    ...
    : allClasses;
  // Future: Let's also support add and delete if we want to expand
  ...
  const handleToggleBelt = (belt: Belt) => {
    ...
    // Limits checking -> max 5 combined
  ...
  return (
    <div className="p-6 md:p-8 flex flex-col bg-krav-bg w-full">
      ...
      <div className="flex gap-6 w-full pb-10 overflow-x-auto">
        <div className="bg-krav-card border border-krav-border rounded-xl flex-1 shadow-sm flex flex-col min-w-[800px]">
          <div className="w-full grid grid-cols-7 border-b border-krav-border bg-black/[0.02]">
            {DAYS.map((day, i) => (
        ...
        {/* Edit Slideover overlay */}
        {editingSession && (
          <div className="max-w-sm w-full shrink-0 flex flex-col bg-krav-card border border-krav-border rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.1)] overflow-hidden h-full z-20 animate-in slide-in-from-right duration-300">
             <div className="p-5 border-b border-krav-border bg-black/5 flex justify-between items-center shrink-0">
               <div>
                  ...
                  <p className="text-xs text-krav-muted mt-0.5">{DAYS[editingSession.dayOfWeek]} às {editingSession.time}</p>
               </div>
             ...
             <form onSubmit={handleSave} className="p-5 flex-1 overflow-y-auto flex flex-col gap-6 bg-krav-bg">
                ...
                <div className="mt-auto pt-6 flex gap-3">
                  ...
                  <button type="button" onClick={() => setEditingSession(null)} className="w-[100px] border border-krav-border bg-krav-card text-krav-text font-bold py-3 text-sm rounded-xl hover:bg-black/5 transition-colors flex items-center justify-center">
                    Cancelar
                  </button>
                </div>
             </form>
